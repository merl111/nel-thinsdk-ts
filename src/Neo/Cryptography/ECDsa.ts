...
export class ECDsa {
  ...
  private static calculateE(
    ...
    return BigInteger.fromUint8Array(
      new Uint8Array(Sha256.computeHash(message)),
      1,
      false
    );
  }
  ...
  public static generateKey(
    curve: ECCurve
  ): { privateKey: ECDsaCryptoKey; publicKey: ECDsaCryptoKey } {
  ...
  public sign(message: ArrayBuffer | ArrayBufferView): ArrayBuffer {
    ...
    do {
      ...
      s = k
        .modInverse(this.key.publicKey.curve.N)
  ...
  private static sumOfTwoMultiplies(
    ...
    for (let i = m - 1; i >= 0; --i) {
      R = R.twice();
      if (k.testBit(i)) {
        if (l.testBit(i)) R = ECPoint.add(R, Z);
        else R = ECPoint.add(R, P);
      } else {
        if (l.testBit(i)) R = ECPoint.add(R, Q);
      }
    }
    return R;
  }
  ...
  public verify(
    ...
    if (
      r.compareTo(this.key.publicKey.curve.N) >= 0 ||
