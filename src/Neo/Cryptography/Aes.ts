export class Aes {
  ...
  public decrypt(ciphertext: ArrayBuffer | ArrayBufferView): ArrayBuffer {
    ...
    for (let i = 0; i < ciphertext_view.length; i += 16)
      this.decryptBlock(
        ciphertext_view.subarray(i, i + 16),
        plaintext.subarray(i, i + 16)
      );
  ...
  public decryptBlock(ciphertext: Uint8Array, plaintext: Uint8Array): void {
    ...
    for (var i = 0; i < 4; i++) {
      ...
      plaintext[4 * i + 1] =
        (Aes.Si[(t[(i + 3) % 4] >> 16) & 0xff] ^ (tt >> 16)) & 0xff;
      plaintext[4 * i + 2] =
        (Aes.Si[(t[(i + 2) % 4] >> 8) & 0xff] ^ (tt >> 8)) & 0xff;
  ...
  public encryptBlock(plaintext: Uint8Array, ciphertext: Uint8Array): void {
    ...
    for (var i = 0; i < 4; i++) {
      ...
      ciphertext[4 * i + 1] =
        (Aes.S[(t[(i + 1) % 4] >> 16) & 0xff] ^ (tt >> 16)) & 0xff;
      ciphertext[4 * i + 2] =
        (Aes.S[(t[(i + 2) % 4] >> 8) & 0xff] ^ (tt >> 8)) & 0xff;
