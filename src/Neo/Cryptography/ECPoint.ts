...
export class ECPoint {
  constructor(
    public x: ECFieldElement,
    public y: ECFieldElement,
    ...
    switch (encoded[0]) {
      ...
      case 0x07: { // hybrid
        ...
        let X1 = BigInteger.fromUint8Array(
          encoded.subarray(1, 1 + expectedLength),
          1,
          false
        );
        let Y1 = BigInteger.fromUint8Array(
          encoded.subarray(1 + expectedLength),
          1,
          false
        );
        p = new ECPoint(
          new ECFieldElement(X1, curve),
          new ECFieldElement(Y1, curve),
          curve
        );
        break;
      }
  ...
  private static decompressPoint(
    ...
    // curve - run!
    //
    if (beta == null) throw new RangeError("Invalid point compression");
  ...
  public static multiply(p: ECPoint, n: Uint8Array | BigInteger): ECPoint {
    ...
    if (preCompLen < reqPreCompLen) {
      ...
      for (let i = 0; i < preCompLen; i++) preComp[i] = oldPreComp[i];
