...
export class ECFieldElement {
  ...
  public add(other: ECFieldElement): ECFieldElement {
    return new ECFieldElement(
      this.value.add(other.value).mod(this.curve.Q),
      this.curve
    );
  }
  ...
  public divide(other: ECFieldElement): ECFieldElement {
    return new ECFieldElement(
      this.value
        .multiply(other.value.modInverse(this.curve.Q))
        .mod(this.curve.Q),
      this.curve
    );
  }
  ...
  public multiply(other: ECFieldElement): ECFieldElement {
    return new ECFieldElement(
      this.value.multiply(other.value).mod(this.curve.Q),
      this.curve
    );
  }
  ...
  public negate(): ECFieldElement {
    return new ECFieldElement(
      this.value.negate().mod(this.curve.Q),
      this.curve
    );
  }
  ...
  public sqrt(): ECFieldElement {
    if (this.curve.Q.testBit(1)) {
      let z = new ECFieldElement(
        BigInteger.modPow(
          this.value,
          this.curve.Q.rightShift(2).add(1),
          this.curve.Q
        ),
        this.curve
      );
    ...
    do {
      ...
      do {
        ...
        P.compareTo(this.curve.Q) >= 0 ||
        !BigInteger.modPow(
          P.multiply(P).subtract(fourQ),
          legendreExponent,
          this.curve.Q
        ).equals(qMinusOne)
      );
  ...
  public square(): ECFieldElement {
    return new ECFieldElement(
      this.value.multiply(this.value).mod(this.curve.Q),
      this.curve
    );
  }
  ...
  public subtract(other: ECFieldElement): ECFieldElement {
    return new ECFieldElement(
      this.value.subtract(other.value).mod(this.curve.Q),
      this.curve
    );
  }
}
