...
export class BinaryReader {
  ...
  public readSerializableArray(T: Function): ISerializable[] {
    ...
    for (let i = 0; i < array.length; i++) array[i] = this.readSerializable(T);
    return array;
  }
  ...
  public readVarInt(max = 9007199254740991): number {
    ...
    let value: number;
    if (fb == 0xfd) value = this.readUint16();
    else if (fb == 0xfe) value = this.readUint32();
    else if (fb == 0xff) value = this.readUint64().toNumber();
    else value = fb;
