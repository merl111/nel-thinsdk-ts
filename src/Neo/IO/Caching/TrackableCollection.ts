...
export class TrackableCollection<TKey, TItem extends ITrackable<TKey>> {
  ...
  public clear(): void {
    this._map.forEach((value, key, map) => {
      if (value.trackState == TrackState.Added) map.delete(key);
      else value.trackState = TrackState.Deleted;
    });
  }

  public commit(): void {
    this._map.forEach((value, key, map) => {
      if (value.trackState == TrackState.Deleted) map.delete(key);
      else value.trackState = TrackState.None;
    });
  }
  ...
  public getChangeSet(): TItem[] {
    ...
    this._map.forEach(value => {
      if (value.trackState != TrackState.None) array.push(value);
    });
    return array;
  }
  ...
  public remove(key: TKey): void {
    ...
    if (item.trackState == TrackState.Added) this._map.delete(key);
    else item.trackState = TrackState.Deleted;
  }
}
