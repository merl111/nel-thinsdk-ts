...
export class MemoryStream extends Stream {
  ...
  public constructor() {
    ...
    if (arguments.length == 0) {
      ...
      if (arguments.length == 1) {
        ...
        this._writable =
          arguments.length == 4 ? (arguments[3] as boolean) : false;
  ...
  private findBuffer(position: number): { iBuff: number; pBuff: number } {
    ...
    let firstSize =
      this._buffers[0] == null ? BufferSize : this._buffers[0].byteLength;
      ...
      while (count > 0) {
        ...
        if (this._buffers[d.iBuff] == null) {
          ...
          actual_count = Math.min(
            count,
            this._buffers[d.iBuff].byteLength - d.pBuff
          );
  ...
  public seek(offset: number, origin: SeekOrigin): number {
    ...
    if (offset < 0 || offset > this._length) throw new RangeError();
  ...
  public setLength(value: number): void {
    if (
      value < 0 ||
      (value != this._length && !this._writable) ||
