...
export class BigInteger {
  ...
  public static get One() {
    return _one || (_one = new BigInteger(1));
  }
  public static get Zero() {
    return _zero || (_zero = new BigInteger(0));
  }
    ...
    if (typeof value === "number") {
      ...
      if (parts.exp <= 0) {
        ...
        for (let i = cu; i < units; i++)
          this._bits[i] =
            parts.man.rightShift(cbit + (i - cu) * DB).toUint32() & DM;
        if (cbit > 0)
  ...
  private clamp(): void {
    let l = this._bits.length;
    while (l > 0 && (this._bits[--l] | 0) == 0) this._bits.pop();
    while (l > 0) this._bits[--l] |= 0;
    if (this._bits.length == 0) this._sign = 0;
  }
  ...
  private static compareAbs(x: BigInteger, y: BigInteger): number {
    ...
    for (let i = x._bits.length - 1; i >= 0; i--)
      ...
      else if (x._bits[i] < y._bits[i]) return -1;
    return 0;
  }
  ...
  public static divRem(
    ...
    if (c == 0)
      return {
        result: sign_result > 0 ? BigInteger.One : BigInteger.MinusOne,
  ...
  public static equals(
    ...
    for (let i = 0; i < bi_x._bits.length; i++)
      if (bi_x._bits[i] != bi_y._bits[i]) return false;
    return true;
  }
  ...
  public static fromUint8ArrayAutoSign(
    arr: Uint8Array,
    littleEndian = true
  ): BigInteger {
  ...
  private fromUint8Array(arr: Uint8Array, sign = 1, littleEndian = true): void {
    if (!littleEndian) {
      ...
      for (let i = 0; i < arr.length; i++) arr_new[arr.length - 1 - i] = arr[i];
      arr = arr_new;
    }
  ...
  private static getDoubleParts(dbl: number) {
    ...
    if (result.exp == 0) {
      if (!result.man.equals(Uint64.Zero)) result.exp = -1074;
  ...
  public getLowestSetBit(): number {
    ...
    for (let x = 0; x < DB; x++)
      if ((this._bits[w] & (1 << x)) > 0) return x + w * DB;
  }
  ...
  public leftShift(shift: number): BigInteger {
    ...
    if (shift == 0) {
      ...
      for (let i = shift_units; i < bits_new.length; i++)
        bits_new[i] =
          ((this._bits[i - shift_units] << shift) |
            (this._bits[i - shift_units - 1] >>> (DB - shift))) &
          DM;
      bits_new[bits_new.length] =
        (this._bits[this._bits.length - 1] >>> (DB - shift)) & DM;
    }
  ...
  public static mod(
    ...
    if (bi_new._sign < 0) bi_new = BigInteger.add(bi_new, bi_y);
    return bi_new;
  }
  ...
  public static modPow(
    value: number | BigInteger,
    exponent: number | BigInteger,
    modulus: number | BigInteger
  ): BigInteger {
    let bi_v = typeof value === "number" ? new BigInteger(value) : value;
    let bi_e =
      typeof exponent === "number" ? new BigInteger(exponent) : exponent;
    ...
    for (let i = 0; i < h; i++) {
      if (i > 0) bi_v = BigInteger.multiply(bi_v, bi_v);
      bi_v = bi_v.remainder(bi_m);
      if (bi_e.testBit(i))
        bi_new = BigInteger.multiply(bi_v, bi_new).remainder(bi_m);
    }
    if (bi_new._sign < 0) bi_new = BigInteger.add(bi_new, bi_m);
    return bi_new;
  }
  ...
  private static multiplyTo(
    ...
    for (let i = x.length + y.length - 2; i >= 0; i--) r[i + offset] = 0;
  ...
  public static pow(value: number | BigInteger, exponent: number): BigInteger {
    ...
    for (let i = 0; i < h; i++) {
      let e = 1 << i;
      if (e > 1) bi_v = BigInteger.multiply(bi_v, bi_v);
      if ((exponent & e) != 0) bi_new = BigInteger.multiply(bi_v, bi_new);
    }
    return bi_new;
  }
  ...
  public rightShift(shift: number): BigInteger {
    ...
    let shift_units = Math.floor(shift / DB);
    shift %= DB;
    if (this._bits.length <= shift_units) return BigInteger.Zero;
    let bits_new = new Array<number>(this._bits.length - shift_units);
    if (shift == 0) {
  ...
  public toUint8Array(littleEndian = true, length?: number): Uint8Array {
    ...
    for (let i = 0; i < array.length; i++) {
      ...
      if (DB - cbits < 8)
        array[offset] =
          ((this._bits[cu] >>> cbits) | (this._bits[cu + 1] << (DB - cbits))) &
          0xff;
      else array[offset] = (this._bits[cu] >>> cbits) & 0xff;
    }
    length = length || BigInteger.getActualLength(array);
    if (length < array.length) array = array.subarray(0, length);

    return array;
  }
  public toUint8ArrayWithSign(
    littleEndian = true,
    length?: number
  ): Uint8Array {
    if (this._sign == 0) return new Uint8Array(length || 1);
    ...
    if (this._sign > 0) {
      ...
      if ((array[array.length - 1] & 0x80) > 0) {
        //最高位为负数
