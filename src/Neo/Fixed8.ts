...
export class Fixed8 implements IO.ISerializable {
  ...
  public static get One() {
    return _one || (_one = Fixed8.fromNumber(1));
  }
  ...
  public add(other: Fixed8): Fixed8 {
    ...
    if (result.compareTo(this.data) < 0) throw new Error();
  ...
  public static fromNumber(value: number): Fixed8 {
    ...
    value *= D;
    if (value >= 0x8000000000000000) throw new RangeError();
  ...
  public static max(first: Fixed8, ...others: Fixed8[]): Fixed8 {
    for (let i = 0; i < others.length; i++)
      if (first.compareTo(others[i]) < 0) first = others[i];
    return first;
  }
  ...
  public static parse(str: string): Fixed8 {
    ...
    str = str.replace(".", "");
    if (digits > 8) str = str.substr(0, str.length - digits + 8);
    else if (digits < 8) for (let i = digits; i < 8; i++) str += "0";
    return new Fixed8(Uint64.parse(str));
  }

  public subtract(other: Fixed8): Fixed8 {
    if (this.data.compareTo(other.data) < 0) throw new Error();
