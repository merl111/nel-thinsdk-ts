...
type PromiseExecutor<T> = (
  resolve: Action<T | PromiseLike<T>>,
  reject: Action<any>
) => void;
...
export class NeoPromise<T> implements PromiseLike<T> {
    ...
    return new NeoPromise<any[]>((resolve, reject) => {
      ...
      let onFulfilled = function(result) {
        ...
        for (let i = 0; i < iterable.length; i++)
          if (iterable[i]._state != PromiseState.fulfilled) return;
        resolve(results);
      };
  ...
  public catch<TResult>(
    onRejected: Func<any, TResult | PromiseLike<TResult>>
  ): PromiseLike<TResult> {
    ...
    if (this._state != PromiseState.pending && this._callback_attached) {
      let callback =
        this._state == PromiseState.fulfilled
          ...
          : this._onRejected;
      let arg =
        this._state == PromiseState.fulfilled ? this._value : this._reason;
      let value, reason;
      try {
      ...
      if (this._next_promise == null) {
        if (reason != null) return NeoPromise.reject(reason);
        else if (value instanceof NeoPromise) return value;
        else return NeoPromise.resolve(value);
      } else {
        if (reason != null) this._next_promise.reject(reason);
        else if (value instanceof NeoPromise)
          value.then(
            this.resolve.bind(this._next_promise),
            this.reject.bind(this._next_promise)
          );
        else this._next_promise.resolve(value);
      }
    }
  }
  ...
  public then<TResult>(
    onFulfilled?: Func<T, TResult | PromiseLike<TResult>>,
    onRejected?: Func<any, TResult | PromiseLike<TResult>>
  ): PromiseLike<TResult> {
