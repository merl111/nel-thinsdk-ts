import {
  GetScriptHashFromScript,
  VerifySignature,
  GetAddressFromPublicKey,
  GetAddressFromScriptHash,
  GetAddressCheckScriptFromPublicKey
} from "./helper";
...
export class Witness {
  ...
  public get Address(): string {
    var hash = GetScriptHashFromScript(this.VerificationScript);
...
export class Transaction {
  ...
  public SerializeUnsigned(writer: Neo.IO.BinaryWriter): void {
    ...
    if (
      this.type == TransactionType.ContractTransaction ||
  ...
  public Deserialize(ms: Neo.IO.BinaryReader): void {
    ...
    if (ms.canRead() > 0) {
      ...
      for (var i = 0; i < witnesscount; i++) {
        ...
        this.witnesses[i].InvocationScript = new Uint8Array(
          ms.readVarBytes()
        ).clone();
        this.witnesses[i].VerificationScript = new Uint8Array(
          ms.readVarBytes()
        ).clone();
      }
    }
  ...
  public AddWitness(
  ...
  ): void {
    {
      ...
      var bsign = VerifySignature(msg, signdata, pubkey);
      if (bsign == false) throw new Error("wrong sign");

      var addr = GetAddressFromPublicKey(pubkey);
      if (addr != addrs) throw new Error("wrong script");
    }

    var vscript = GetAddressCheckScriptFromPublicKey(pubkey);
  ...
  public AddWitnessScript(vscript: Uint8Array, iscript: Uint8Array): void {
    var scripthash = GetScriptHashFromScript(vscript);
    if (this.witnesses == null) this.witnesses = [];
