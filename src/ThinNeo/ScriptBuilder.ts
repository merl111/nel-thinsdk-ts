import { String2Bytes, GetPublicKeyScriptHash_FromAddress } from "./helper";
...
export class ScriptBuilder {
  ...
  _WriteUint8Array(nums: Uint8Array): void {
    for (var i = 0; i < nums.length; i++) this.writer.push(nums[i]);
  ...
  public Emit(op: any, arg: Uint8Array = null): ScriptBuilder {
    this._WriteUint8(op);
    if (arg != null) this._WriteUint8Array(arg);
    return this;
  }
  ...
  public EmitAppCall(
    ...
    if (scriptHash.length != 20) throw new Error("error scriptHash length");
    return this.Emit(
  ...
  public EmitJump(op: any, offset: number): ScriptBuilder {
    if (
      op != OpCode.JMP &&
      op != OpCode.JMPIF &&
      op != OpCode.JMPIFNOT &&
  ...
  public EmitPushBytes(data: Uint8Array): ScriptBuilder {
    if (data == null) throw new Error("ArgumentNullException");
  ...
  public EmitPushString(data: string): ScriptBuilder {
    return this.EmitPushBytes(String2Bytes(data));
  }

  public EmitSysCall(api: string): ScriptBuilder {
    if (api == null) throw new Error("ArgumentNullException");
    var api_bytes = String2Bytes(api);
  ...
  public EmitParamJson(param: any): ScriptBuilder {
    if (typeof param === "number") {
      ...
      if (str[0] != "(")
        throw new Error(
          "must start with:(str) or (hex) or (hexrev) or (addr)or(int)"
        );
      ...
      //(address) or(addr)开头，表示是一个地址，转换为脚本hash
        ...
        var hex = GetPublicKeyScriptHash_FromAddress(addr);
        this.EmitPushBytes(hex);
      ...
      //(int256) or (hex256) 开头,表示是一个定长的256位 16进制大整数
        ...
        var hex = str.substr(8).hexToBytes();
        if (hex.length != 32) throw new Error("not a int256");
        this.EmitPushBytes(hex.reverse());
      }
      //(int160) or (hex160) 开头,表示是一个定长的160位 16进制大整数
      else if (str.indexOf("(int160)") == 0 || str.indexOf("(hex160)") == 0) {
        var hex = str.substr(8).hexToBytes();
        if (hex.length != 20) throw new Error("not a int160");
        this.EmitPushBytes(hex.reverse());
      } else
        throw new Error(
          "must start with:(str) or (hex) or (hexbig) or (addr) or(int)"
        );
    } else {
